...
void app_main(void)
{
    ...
    // i2s_cfg.type = AUDIO_STREAM_WRITER;                                          AQUI
    ...
    periph_touch_cfg_t touch_cfg = {
        .touch_mask = TOUCH_SEL_SET | TOUCH_SEL_PL\AY | TOUCH_SEL_VOLUP | TOUCH_SEL_VOLDWN,
    ...
    while (1) {
        ...
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT && msg.source == (void *) bt_stream_reader
            ...
            // printf("music_info.uri: %s \n", music_info.uri);
            // ESP_LOGI(TAG, "[ * ] Receive music info from Bluetooth, %s",
            //          music_info.uri);
